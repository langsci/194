...
Now, in order to be able to visually present the specific geographic position of each of the Dutch lexemes in the rows, their position in the n-dimensional space is reduced to a two-dimensional space. All five Dutch lexemes can be then represented as points in this space (\figref{fig:3:17}):
...
When CA is applied to the data sets gathered for this study, a first visualization via CA of the SourceDutch field of \textit{beginnen} looks is obtained (\figref{fig:3:19}):
...
\includegraphics[height=.3\textheight]{figures/tree28.pdf}
Chaining effect
...
\includegraphics[height=.3\textheight]{figures/tree29.pdf}
Space dilation
...
\begin{table}\caption{Table~13  Combinatory possibilities of the selected distance measures, clustering algorithms and ``spatial maps''. \textsuperscript{†}: (+ high space dilation)\label{tab:3:13}}
\begin{tabular}{llrlr} 
\lsptoprule
